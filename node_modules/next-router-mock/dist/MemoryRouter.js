...
function parseUrlToCompleteUrl(url, currentPathname) {
    var _a;
    const parsedUrl = typeof url === "object" ? url : urls_1.parseUrl(url);
    return {
...
function getRouteAsPath(pathname, query, hash) {
    ...
    const qs = urls_1.stringifyQueryString(remainingQuery);
    if (qs)
        asPath += `?${qs}`;
    if (hash)
        asPath += hash;
...
function shouldTriggerHashChange(current, newRoute) {
    ...
    const isQueryChange = urls_1.stringifyQueryString(current.query) !== urls_1.stringifyQueryString(newRoute.query);
